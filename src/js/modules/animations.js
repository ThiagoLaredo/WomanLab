...
//     });
//   }
//  // Animação para o conteúdo dinâmico específico após ser carregado
//  const dynamicContentElements = document.querySelectorAll('#dynamic-content *');
//  if (dynamicContentElements.length > 0) {
...
//   // Animação para elementos fixos na página
//   gsap.set([".header", "[data-menu='logo']", "#menu > li", ".social-icons-header a", ".img-circulo", ".introducao-texto h1"], { opacity: 0 });
...
// Animações para conteúdo dinâmico
export const animateDynamicContent = () => {
  const dynamicContentElements = document.querySelectorAll('#dynamic-content *');
  if (dynamicContentElements.length > 0) {
    dynamicContentElements.forEach(element => {
      gsap.from(element, {
